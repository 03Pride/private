...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	if(hcan == &hcan1)
	{
		...
		switch (Rx1Message.StdId)
		{
			...
			{
//				powercontrol.supercap_energy =  CAN1_Rx_data[0] << 8 | CAN1_Rx_data[1];
//				powercontrol.supercap_power  =  CAN1_Rx_data[2] << 8 | CAN1_Rx_data[3];
			}
			break;
			default:
			{
	...
	else if(hcan == &hcan2 )
	{
		...
		{	        
			...
			case CAN_3508_M4_ID:
			{
				...
				moto_chassis[i].msg_cnt++ <= 50 ? get_moto_offset(&moto_chassis[i], &hcan2) : encoder_data_handler(&moto_chassis[i],&hcan2,CAN2_Rx_data);
			}
			break;
			case CAN_YAW_MOTOR_ID:
			{
				encoder_data_handler(&moto_yaw, hcan,CAN2_Rx_data);
			}
			break;
			case CAN_PIT_MOTOR_ID:
			{
